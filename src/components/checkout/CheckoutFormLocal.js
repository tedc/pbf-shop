...
import { createCheckoutData, cleanCart } from "../../functions";
import OrderSuccess from "./OrderSuccess";
import FormCouponLocal from '../cart/cart-page/FormCouponLocal';
...
const CheckoutFormLocal = ({countriesData, gateways, stripe, elements}) => {
    ...
    const { setMiniCart, miniCart } = useContext(AppContext);
    ...
    const handleFormSubmit = async (event) => {
        ...
        if ( 'stripe' === input?.paymentMethod ) {
            const createdOrderData = await handleStripeCheckout(input, miniCart?.products, setRequestError, setIsStripeOrderProcessing, setCreatedOrderData, session);
            router.push(`/thank-you?order_id=${createdOrderData?.orderId}`);
            return null;
        } else if( 'paypal' === input?.paymentMethod ) {
            const createdOrderData = await handlePayPalCheckout(input, miniCart?.products, setRequestError, setIsStripeOrderProcessing, setCreatedOrderData, session);
                    ...
                    <div className="column column--s10-lg">
                        <form onSubmit={handleFormSubmit} className={cx('form', 'form--main', 'form--checkout', 'columns', 'columns--jcsb', {'form--loading' : isOrderProcessing})}>
                            ...
                            <div className="column column--grow-60-bottom column--s4-md column--s3-lg">
                                <FormCouponLocal cart={miniCart} setRequestError={setRequestError} setMiniCart={setMiniCart} />
                                {/* Order & Payments*/}
...
export default CheckoutFormLocal;
