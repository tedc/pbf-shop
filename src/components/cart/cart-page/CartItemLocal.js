...
const CartItem = ( {
    ...
    const { setCartLoading, cartFetching, setCartFetching, setMiniCart } = useContext(AppContext);
    ...
    return (
        ...
        {   isMiniCart ? (
                <div className="mini-cart__item" key={ item.productId }>
                    ...
                    <span className="remove"
                      onClick={ ( event ) => handleRemoveProductClick( item?.productId ) }></span>
                </div>
            )
        : 
        (<tr className="tr" key={ item.productId }>
            ...
            <td className="td">
                { ( 'string' !== typeof item.totalPrice ) ? stringifyPrice( item.totalPrice.toFixed( 2 ) ) : item.totalPrice }
            </td>
            <th className="td td--remove">
                {/* Remove item */}
                <span className="remove"
                      onClick={ ( event ) => handleRemoveProductClick( event, products, item?.productId ) }>
